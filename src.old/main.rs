...
pub struct PlayerScores {
    pub player_scores: HashMap<Uuid, usize>,
    pub player_alive: HashMap<Uuid, bool>,
}
...
async fn main() {
    ...
    let zebedee_client = {
        ...
        ZebedeeClient::new().apikey(api_key).build()
    };
    ...
    let player_scores = Arc::new(RwLock::new(PlayerScores::new()));

    let global_state_clone = Arc::clone(&global_state);
    let player_scores_clone = Arc::clone(&player_scores);

    tokio::spawn(
        async move { server_loop(tick_tx, global_state_clone, player_scores_clone).await },
    );

    let global_state = warp::any().map(move || global_state.clone());
    let player_scores = warp::any().map(move || player_scores.clone());
    ...
    let routes = health_check.or(warp::path("run")
        ...
        .and(global_state)
        .and(player_scores)
        .map(|ws: warp::ws::Ws, global_state, player_scores| {
            ws.on_upgrade(move |socket| new_websocket(socket, global_state, player_scores))
        }));
